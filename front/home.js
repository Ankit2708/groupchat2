...
window.addEventListener('DOMContentLoaded',(e)=>{
    ...
    .then(user=>{
        ...
        if(admin){
            ...
            adminDiv.innerHTML=`<li><a href="./admin.html" id="leaderboard>Admin Controls</a></li>`
        }
        axios.get(`http://localhost:3000/getmessage?grpId=${grpId}`,{headers:{"Authorization":token}})
        .then(ress=>{
            const messages=ress.data.messages
...
sendBtn.addEventListener('click',(e)=>{
    ...
    if(message.value=''){
        ...
        axios.post('http://localhost:3000/addmessage',{groupid:grpId,msg:message.value},{
            headers:{"Authorization":token}
        }).then(ress=>{
